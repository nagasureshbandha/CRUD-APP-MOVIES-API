...
const convertDirectorObjectToResponsiveObject = (dbObject) => {
  return {
    directorId: dbObject.director_id,
...
FROM
 ...
 movie_id = ${movieId};`;
  ...
  response.send(convertMovieObjectToResponsiveObject(movie));
});
...
app.get("/directors/", async (request, response) => {
  const getDirectorsQuery = `SELECT * FROM 
    director;`;

  const directorsArray = await db.all(getDirectorsQuery);
  response.send(
    directorsArray.map((eachDirector) =>
      convertDirectorObjectToResponsiveObject(eachDirector)
    )
  );
});
